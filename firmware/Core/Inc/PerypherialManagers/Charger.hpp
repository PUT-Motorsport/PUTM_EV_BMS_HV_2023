...
public:
	...
	{
		return HAL_FDCAN_AddMessageToTxFifoQ(&hfdcan2,
...
class ChargerCanRxMessageHandler
{
	...
	uint32_t can_fifo_message_count() const
	{
		return HAL_FDCAN_GetRxFifoFillLevel(&hfdcan2, FDCAN_RX_FIFO0);
	}

	inline void process_frame()
	{
		FDCAN_RxHeaderTypeDef header{};
		auto status = HAL_FDCAN_GetRxMessage(&hfdcan2, FDCAN_RX_FIFO0, &header, data.begin());
...
public:
	inline void update()
	{
		while (can_fifo_message_count() not_eq 0)
		{
			process_frame();
		}
	}

	constexpr float get_battery_voltage() const
	{
		return battery_read_voltage;
	}

	constexpr float get_battery_current() const
	{
